...
import { AutoRulesFields, type AutoRulesDraft } from "@/components/auto-rules-fields";

function todayStr(): string {
  return new Date().toISOString().slice(0, 10);
}

export function NewTimeSimForm() {
  ...
  const [autoRules, setAutoRules] = useState<AutoRulesDraft>({
    stopLossPct: 5,
    takeProfitPct: 10,
    ruleScope: "auto_only",
    reinvestStrategy: "oversold",
    duplicateHandling: "skip_to_next",
    noMatchBehavior: "hold_cash",
  });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/timesims", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          maxPositionPct: maxPositionPct
            ...
            : undefined,
          autoRules: autoRulesEnabled
            ? { ...autoRules, universe: "sp500" }
            : undefined,
        }),
      });
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-ink-dim">
          Set up an isolated portfolio for dated buys & sells. After creating
          you&apos;ll be able to add trades on specific historical dates and
          scrub the as-of date to see how the portfolio looked at any point.
        </p>
      </div>
      ...
      <form onSubmit={submit} className="space-y-5">
        <Field label="Name" required>
          <input
            ...
            placeholder="e.g., 2024 Tech rebound backtest"
        ...
        <Field
          label="Description"
          hint="Optional — what's the thesis or scenario being tested?"
        >
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="Bought MAG7 on 2023-10-30 lows, trimmed half on 2024-07-10, full exit at year-end"
            rows={3}
            maxLength={500}
            className="w-full rounded-md border border-edge bg-bg-elev2 px-3 py-2 text-sm focus:border-signal-accent focus:outline-none"
          />
        ...
        <Field
          label="Start date"
          hint="When the scenario starts — the default date when you add trades, and the lower bound for the as-of slider."
          required
        >
          <input
            type="date"
        ...
        <div className="rounded-lg border border-edge bg-bg-elev p-4">
          <label className="flex cursor-pointer items-center gap-2">
            <input
              ...
              onChange={(e) => setAutoRulesEnabled(e.target.checked)}
              className="h-4 w-4 rounded border-edge accent-signal-accent"
            />
            <Zap className="h-4 w-4 text-signal-accent" />
            <span className="text-sm font-medium">
              Enable rule-based auto-trading
            </span>
          </label>
          <p className="mt-1 ml-6 text-xs text-ink-dim">
            Engine sells positions hitting your stop-loss or take-profit,
            then redeploys the cash into the top S&P 500 pick from a scanner
            strategy. You can run it from the sim&apos;s detail page.
          </p>
          {autoRulesEnabled && (
            <div className="mt-4 ml-6 border-l border-edge pl-4">
              <AutoRulesFields value={autoRules} onChange={setAutoRules} />
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          >
            {submitting ? "Creating..." : "Create time simulation"}
          </button>
        </div>
      </form>
    </div>
