...
2. For each `.json` file in that folder:
   ...
   b. Use **web search** to find recent earnings reports, guidance, analyst notes, material news, and overall market sentiment from the last 90 days for the ticker. Use the company name and ticker in your queries. Aim for 2–4 searches per stock.
...
3. At the end, output a concise summary: how many analyses you wrote, which tickers, the three scores per ticker, and any failures (with reasons). DO NOT REFER TO ANY PREVIOUS ANALYSES YOU DID IN PAST RUNS. Each run is independent and should only report on the requests it processed this time.
